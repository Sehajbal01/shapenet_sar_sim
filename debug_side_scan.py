...
SONAR_BASELINE, imported rather than copied so the debug output cannot drift away from the
...
from sonar_paper_figures import SONAR_BASELINE
...
if __name__ == '__main__':
    ...
    for pose_num in pose_nums:
        debug_side_scan(
            ...
            pose_num = pose_num,
            num_pings  = 256,   # 4x SONAR_BASELINE's 64
            spatial_fs = 64,    # 2x SONAR_BASELINE's 32
            spatial_bw = 128,   # 2x SONAR_BASELINE's 64
            region_radius = 2.0,  # 2x SONAR_BASELINE's 1.0
            ...
            # compression/db_floor/asinh_k_ratio come from SONAR_BASELINE unless overridden here
